...
{
	public partial class _2010guangzhou_chezhan_Serial : beijing_2010_PageBase
	{
		...
		{
			int pavilionId = ConvertHelper.GetInteger(_SerialXmlNode.ParentNode.ParentNode.Attributes["PavilionId"].Value.ToString());
			StringBuilder pageGuilder = new StringBuilder("");
			pageGuilder.AppendFormat("<em>&gt;</em><a href=\"{2}{0}/\">{1}</a></span><em>&gt;</em><span>"
									, PavilionUrl[pavilionList[pavilionId].Name.Trim()]
									, pavilionList[pavilionId].Name
									, _PavilionUrl);
			pageGuilder.AppendFormat("<a href=\"" + _MasterBrandUrl + "\">{1}</a></span><em>&gt;</em>"
		...
		//	masterBrandTitle.Append("</div>");
		//	masterBrandTitle.Append("</div>");
		//	masterBrandTitle.Append("<div class=\"mbrand\"></div>");
		//	masterBrandTitle.Append("</div>");
		...
		//	return base.GetCommonNavigation("CsGuangZhou2010", int.Parse(_SerialXmlNode.GetAttribute("ID").ToString()));
		//}
		/// <summary>
		/// 焦点图
		/// </summary>
		...
		protected string PageFocusChart()
		{
			...
			focusString.AppendFormat(" <li><strong>厂家：</strong><a target=\"_blank\" href=\"" + _MasterBrandUrl + "\">{1}</a>"
									, _SerialXmlNode.ParentNode.ParentNode.Attributes["AllSpell"].Value.ToString()
									, _SerialXmlNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());
			...
			focusString.AppendFormat("<li class=\"r\"><strong>展馆：</strong><a target=\"_blank\" href=\"{0}/\">{1}</a>"
		...
		protected string CarTypeImage()
		{
			...
			string sPath = WebConfig.DataBlockPath + "Data\\CarShow\\" + _SaveNewsUrl + "\\SerialImages.xml";
		...
		protected string OtherCarType()
		{
			...
			otherString.AppendFormat("<h3><span><a target=\"_blank\" href=\"" + _MasterBrandUrl + "\">{1}车型</a></span></h3>"
									, _SerialXmlNode.ParentNode.ParentNode.Attributes["AllSpell"].Value.ToString()
									, _SerialXmlNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());
		...
		protected string PageNewSpan()
		{
			...
			int index = 0;
			Dictionary<string, ExhibitionNew> eNewList = GetExhibitionNewsList("serial"
												, ConvertHelper.GetInteger(_SerialXmlNode.GetAttribute("ID"))
												, _SaveNewsUrl);
			...
			string spath = WebConfig.DataBlockPath + "Data\\CarShow\\" + _SaveNewsUrl + "\\Serial\\Carshow_Serial_"
		...
		protected string PageComments()
		{
			...
			string sPath = WebConfig.DataBlockPath
						   + "Data\\CarShow\\" + _SaveNewsUrl + "\\SerialDianping\\" + "Dianping_Serial_"
						   + _SerialXmlNode.GetAttribute("ID") + ".xml";
			...
			foreach (XmlElement xElem in xNodeList)
			{
				...
				else
				{
					serialSpan.AppendFormat("<p class=\"price\"><label>价格：</label><a href=\"{0}\" target=\"_blank\">{1}</a></p>"
							, string.Format("http://price.bitauto.com/frame.aspx?newbrandid={0}"
							, serialId), priceString);
				}
				//生成密钥集
