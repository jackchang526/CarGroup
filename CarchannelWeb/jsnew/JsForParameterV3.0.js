...
var ComparePageObject = {
	...
	MenuOffsetTop: 130, //滚动菜单 相对车款头的高度偏移量
...
function pullTopAd(adHeight) {
	...
	$('[data-spy="scroll"]').each(function () {
		var $spy = $(this);
		//		var offsetH = $spy.data("offset");
		//		$spy.data("offset", offsetH - adHeight);
		...
		$.ajax({
			url: url, dataType: 'jsonp', jsonp: "XSS_HTTP_REQUEST_CALLBACK", jsonpCallback: "errorCallback", success: function (data) {
				if (data.status == "success") {
					$("#popup-box").hide();
					$("div[class='no-txt-box have-txt-box']").css("background", "url(http://img4.bitautoimg.com/uimg/index2014/images2/success_ico.jpg) no-repeat left center");
					$("#popup-box-success").find(".tit").html("提交成功！").siblings("p").html("您提交的纠错信息我们已经收集到，感谢您的纠错。");
					$("#popup-box-success").showCenter();
				} else {
					$("div[class='no-txt-box have-txt-box']").css("background", "url(http://img3.bitautoimg.com/uimg/index2014/images2/error_ico.jpg) no-repeat left center");
					$("#popup-box-success").find(".tit").html("提交失败！").siblings("p").html(data.message);
					$("#popup-box-success").showCenter();
				}
			}
		});
		...
		if (checkCarIsShowForeach(i)) {
			if (ComparePageObject.ArrCarInfo[i].CarInfoArray) {
				...
				try {
					...
					if (referPrice != "无") {
						ComparePageObject.ArrPageContent.push("<dd class=\"car-price\"><a target=\"_blank\" href=\"/" + csAllSpell + "/m" + carid + "/baojia/\">" + referPrice + "</a> " + (ComparePageObject.ArrCarInfo[i].CarInfoArray[0][9] == "待销" ? "(预售价)" : "(厂家指导价)") + "</dd>");
					}
					ComparePageObject.ArrPageContent.push("</dl>");
					...
					if (referPrice != "无") {
						ComparePageObject.FloatTop.push("<dd class=\"car-price\"><a target=\"_blank\" href=\"/" + csAllSpell + "/m" + carid + "/baojia/\">" + referPrice + "</a>" + (ComparePageObject.ArrCarInfo[i].CarInfoArray[0][9] == "待销" ? "(预售价)" : "(厂家指导价)") + " </dd>");
					}
					...
					if (ComparePageObject.IsNeedDrag) {
						...
						var compareStr = "";//是否已经加入对比
						if (com_new_arr.contains(carid)) {
							compareStr = "<div class=\"button_gray btn-compare-car add\" id=\"carcompare_btn_new_" + carid + "\" ><a href=\"javascript:;\"><span>已加入</span></a></div>";
						}
						else {
							compareStr = "<div class=\"button_gray btn-compare-car\" id=\"carcompare_btn_new_" + carid + "\"><a title=\"对比\" href=\"javascript:;\" cid=\"" + carid + "\"><span>对比</span></a></div>";
						}
						if (isShowLoop == 0) {
							if (checkIsRightEnd(i)) {
								...
								ComparePageObject.ArrPageContent.push("<div class=\"button_gray btn-compare-car\" id=\"carcompare_btn_new_" + carid + "\"><span>对比</span></a></div>");
								ComparePageObject.ArrPageContent.push(compareStr);
							...
							if (checkIsRightEnd(i)) {
								...
								ComparePageObject.ArrPageContent.push("<span class=\"button_gray btn-pre-car\"><a href=\"javascript:;\" title=\"左移\"><</a></span>" + compareStr + "<span class=\"button_gray btn-next-car\"><a href=\"javascript:;\" class=\"btn-next-car\" title=\"右移\">></a></span>");
	...
	if (currScrollLeft > boxOffsetLeft) {
		...
		if (currScrollTop > boxOffsetTop) {
			if (ComparePageObject.IE6) {
				$("#left-nav").css({ position: "absolute", top: currScrollTop - 275, left: currScrollLeft - boxOffsetLeft });
		...
		if (currScrollTop > boxOffsetTop) {
			if (ComparePageObject.IE6) {
				$("#left-nav").css({ position: "absolute", top: currScrollTop - 275, left: -ComparePageObject.MenuOffsetLeft + "px" });
			}
			else {
			...
			if ($("#left-nav ul li").length > 0) {
				$("#left-nav").css({ position: "absolute", top: ComparePageObject.MenuOffsetTop + "px", left: -ComparePageObject.MenuOffsetLeft + "px" });
			}
		}
		$("#show-left-nav").hide();
	}
...
$(function () {
	if (isie(6)) {
		//$("left-nav").find("i").first().show();
		//alert($("left-nav").find("i").first().parent().html());
          ...
          .map(function (i, n) {
			 ...
			 $targetElement = $("#" + targetName);
              ...
              && ($targetElement.length > 0)) {
          		var ElementScrollTop = ($targetElement.offset().top + self.options.offset - (i * (targetHeight + 1)));
          		//console.log($targetElement.offset().top + "|" + self.options.offset + "|" + (i * (targetHeight + 1)) + "|" + targetHeight);
          		//setTimeout(function () { console.log($el, $el.get(0).offsetHeight); }, 3000);
