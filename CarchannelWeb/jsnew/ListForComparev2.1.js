// 车型及子品牌列表 添加对比用
var ListForCompare = {
    DataPath: "http://car.bitauto.com/car/ajaxnew/ListForComparev2.1.ashx?type=",     // 数据源位置
    ...
    CarListDataPath: "http://car.bitauto.com/car/ajaxnew/SerialToCarForCompare.aspx",
...
function getSerialListBySelectLevel(index, type) {
    ...
    // 更新样式
    for (var i = 0; i < LevelForRange.length; i++) {
        if (!document.getElementById("SelectLiForLevel_" + LevelForRange[i].Type))
        { continue; }
        if (LevelForRange[i].Type != type) {
            document.getElementById("SelectLiForLevel_" + LevelForRange[i].Type).className = "";
        }
        else {
            document.getElementById("SelectLiForLevel_" + LevelForRange[i].Type).className = "current";
        }
    }
}
...
function getAndShowCarListForSerial(carList) {
    ListForCompare.CarListDivObject.innerHTML = "<iframe style=\"z-index:-1;position:absolute;width:100%;height:400px;_filter:alpha(opacity=0);opacity=0;border-style:none;\"></iframe>" + carList;
