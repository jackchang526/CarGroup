...
var ComparePageObject = {
	...
	MenuOffsetTop: 150, //滚动菜单 相对车款浮动头的高度偏移量
	MenuOffsetLeft: 95//滚动菜单 相对表格 左偏移量
}
	...
	for (var i = 0; i < 4 && i < arrCarId.length; i++) {
		$.ajax({
			url: "http://api.car.bitauto.com/carinfo/carinfojson.ashx?action=baseinfo&carid=" + arrCarId[i], cache: true, dataType: 'jsonp', jsonpCallback: "adCallback" + i, success: function (data) {
				...
				//arrHtml.push("<li>");
				//arrHtml.push("<a href=\"/" + data.AllSpell + "/\" target=\"_blank\"><img src=\"" + data.ImageUrl + "\"></a>");
				//arrHtml.push("<dl>");
				//arrHtml.push("<dt><a target=\"_blank\" href=\"/" + data.AllSpell + "/\">" + data.SerialName + "</a></dt>");
				//arrHtml.push("<dd><a target=\"_blank\" href=\"/" + data.AllSpell + "/m" + data.CarId + "/\">" + data.CarName + "</a></dd>");
				//arrHtml.push("</dl>");
				//arrHtml.push("<div class=\"rightfixed-btn-duibi button_gray\">");
				//arrHtml.push("<a target=\"_self\" href=\"javascript:addCarToCompareForSelect(" + data.CarId + ",'','');\"><span>对比</span></a>");
				//arrHtml.push("</div>");
				//arrHtml.push("</li>");
...
function bindEvent() {
	...
	$("#popup-box .btn-close,#btnErrorCancel").click(function () { $("#popup-box").hide(); });
	$("#popup-box-success .btn-close,#btn-success-close").click(function () { $("#popup-box-success").hide(); });
	...
	$(document).click(function (e) {
		...
		//换车
		if (($(target).closest(".popup-change-car").length <= 0) && ($(target).closest("span[id^='change_car_']").length <= 0)) {
			$(".change-car-area span[id^='change_car_']").removeClass("btn-hide-car").addClass("btn-show-car").siblings(".popup-change-car").hide();
		}
		//为我推荐
		if ($(target).closest(".popup-recom").length <= 0 && ($(target).closest(".btn-hide-car").length <= 0)) {
			$(".recom-car > span:first-child").removeClass("btn-hide-car").addClass("btn-show-car").siblings(".popup-recom").hide();
		}
	});
}
//已选车款不可选
...
function createPic() {
	...
	ComparePageObject.FloatTop.push("<th class=\"pd0 bt\">");
	...
	ComparePageObject.FloatLeft.push("<td class=\"pd0 bt\">");
	...
	ComparePageObject.ArrPageContent.push("<th class=\"bt pd0" + classNameAdd + "\">");
	...
	for (var i = 0; i < ComparePageObject.ValidCount; i++) {
		...
		if (i == 0)
		{ ComparePageObject.ArrPageContent.push("<div id=\"tableHead_" + i + "\" class=\"tableHead_item y2015\">"); }
		else
		{ ComparePageObject.ArrPageContent.push("<div id=\"tableHead_" + i + "\" class=\"tableHead_item y2015\">"); }
		...
		ComparePageObject.ArrPageContent.push("<div style=\"height:187px\">");
		...
		ComparePageObject.FloatTop.push("<div id=\"FloatTop_tableHead_" + i + "\" class=\"tableHead_item y2015\">");
		ComparePageObject.FloatTop.push("<div class=\"sel-car-box\">");
		ComparePageObject.FloatTop.push("<div class=\"dl-box\" style=\"height:109px;\"><div  id=\"FloatTop_carBox_" + i + "\" class=\"dl-box\">");
		...
		ComparePageObject.ArrPageContent.push("<span id=\"change_car_" + i + "\" class=\"button_gray btn-show-car\"><a href=\"javascript:setSelectControlForCompare('tableHead_" + i + "','HeadTemp')\"><b>换辆车</b></a></span>");
		// for FloatTop
		ComparePageObject.FloatTop.push("<span id=\"change_car_float_" + i + "\" class=\"button_gray btn-show-car\"><a href=\"javascript:setSelectControlForCompare('FloatTop_tableHead_" + i + "','Float_HeadTemp')\"><b>换辆车</b></a></span>");
	...
	if (ComparePageObject.NeedBlockTD > 0) {
		for (var i = 0; i < ComparePageObject.NeedBlockTD; i++) {
			...
			if (i == 0) {
				ComparePageObject.ArrPageContent.push("<div id=\"tableHead_" + index + "\" class=\"tableHead_item y2015\" bit-disabled=\"\">");
			}
			else {
				ComparePageObject.ArrPageContent.push("<div id=\"tableHead_" + index + "\" class=\"tableHead_item y2015 noBg\">");
			}
			//ComparePageObject.ArrPageContent.push("<div class=\"top-txt\">车款" + (index + 1) + "</div>");
			if (i == 0) {
				ComparePageObject.ArrPageContent.push("<div id=\"master4\" class=\"sel-item-box z3\" style=\"z-index: 50\"></div>");
				ComparePageObject.ArrPageContent.push("<div id=\"serial4\" class=\"sel-item-box z2\" style=\"z-index: 40\"></div>");
				ComparePageObject.ArrPageContent.push("<div id=\"cartype4\" class=\"sel-item-box z1\" style=\"z-index: 30\"></div>");
				if (index > 0)
					ComparePageObject.ArrPageContent.push("<div class=\"recom-car\"><span class=\"button_gray btn-show-car\"><a href=\"javascript:recomCarList('tableHead_" + index + "'," + (index - 1) + ");\"><b>为我推荐</b></a></span></div>");

				// for FloatTop
				ComparePageObject.FloatTop.push("<td class=\"pd0\" style=\"position:relative; z-index:" + (ComparePageObject.NeedBlockTD - i) + "\">");
				ComparePageObject.FloatTop.push("<div id=\"FloatTop_tableHead_" + index + "\" class=\"tableHead_item y2015\" bit-disabled=\"\">");

				ComparePageObject.FloatTop.push("<div class=\"recom-car\"><span class=\"button_gray btn-show-car\"><a href=\"javascript:recomCarList('FloatTop_tableHead_" + index + "'," + (index - 1) + ");\"><b>为我推荐</b></a></span></div>");

				ComparePageObject.FloatTop.push("</div>");
				ComparePageObject.FloatTop.push("</td>");
			}
			else {
				ComparePageObject.ArrPageContent.push("<div class=\"sel-item-box z3\" style=\"z-index: 50\"><span class=\"sel-item sel-item-disabled\"><span>请选择品牌</span><em></em></span></div>");
				ComparePageObject.ArrPageContent.push("<div class=\"sel-item-box z2\" style=\"z-index: 40\"><span class=\"sel-item sel-item-disabled\"><span>请选择车型</span><em></em></span></div>");
				ComparePageObject.ArrPageContent.push("<div class=\"sel-item-box z1\" style=\"z-index: 30\"><span class=\"sel-item sel-item-disabled\"><span>请选择车款</span><em></em></span></div>");


				// for FloatTop
				ComparePageObject.FloatTop.push("<td class=\"pd0\" style=\"position:relative; z-index:" + (ComparePageObject.NeedBlockTD - i) + "\"><div class=\"tableHead_item y2015 noBg\">");
				ComparePageObject.FloatTop.push("<div class=\"carBox\"><div class=\"sel-item-box z3\" style=\"z-index: 50\"><span class=\"sel-item sel-item-disabled\"><span>请选择品牌</span><em></em></span></div><div class=\"sel-item-box z2\" style=\"z-index: 40\"><span class=\"sel-item sel-item-disabled\"><span>请选择车型</span><em></em></span></div><div class=\"sel-item-box z1\" style=\"z-index: 30\"><span class=\"sel-item sel-item-disabled\"><span>请选择车款</span><em></em></span></div></div>");
...
function setSelectControlForCompare(objID, suffix) {
	if (objID.indexOf('_') >= 0) {
		...
		var btnChangeObj = $("#" + objID + " .change-car-area span[id^='change_car_']").eq(0);
		if (btnChangeObj.hasClass("btn-show-car")) {
			$("#" + objID + " .change-car-area .popup-change-car").remove();
			btnChangeObj.removeClass("btn-show-car").addClass("btn-hide-car");
			$("#" + objID).parent().siblings().find(".popup-change-car").hide().parent().children("span[id^='change_car_']").removeClass("btn-hide-car").addClass("btn-show-car");
			$("#" + objID).parent().siblings().find(".popup-recom").hide().parent().children("span:first").removeClass("btn-hide-car").addClass("btn-show-car");
			//设置为我推荐设置zindex
			...
			$("#" + objID + " .change-car-area span[id^='change_car_']").eq(0).removeClass("btn-hide-car").addClass("btn-show-car");
...
function recomCarList(objId, prevIndex) {
	...
	var btnRecomObj = $("#" + objId + " .recom-car span:first");
	if (btnRecomObj.hasClass("btn-show-car")) {
		btnRecomObj.removeClass("btn-show-car").addClass("btn-hide-car");
		//向左显示 父级zindex设置最大
		if (rightWidth < fixWidth) $("#" + objId).parent().css({ 'z-index': 11 });
	}
	else if (btnRecomObj.hasClass("btn-hide-car")) {
		btnRecomObj.removeClass("btn-hide-car").addClass("btn-show-car");
	...
	tempHmtl.push("<div class=\"button_orange button_112_26\"><a href=\"javascript:addAllCarToCompare('" + objId + "'," + prevIndex + ");\" class=\"btn-gray-big\">全部加入对比</a></div>");
	tempHmtl.push("</div>");
...
function mathLeftMenu(currScrollLeft, boxOffsetLeft, currScrollTop, boxOffsetTop) {
	...
	if (currScrollLeft > boxOffsetLeft) {
		...
		if (currScrollTop > boxOffsetTop) {
			if (ComparePageObject.IE6) {
				$("#left-nav").css({ position: "absolute", top: currScrollTop - 45, left: currScrollLeft - boxOffsetLeft });
				$("#show-left-nav").css({ position: "absolute", top: currScrollTop - 45, left: currScrollLeft - boxOffsetLeft });
			}
			else {
			...
			//232px 车型对比 包括图片 头高度
			$("#left-nav").css({ position: "absolute", top: "232px", left: currScrollLeft - boxOffsetLeft });
			$("#show-left-nav").css({ position: "absolute", top: "232px", left: (currScrollLeft - boxOffsetLeft) });
		}
	}
	else {
		...
		if (currScrollTop > boxOffsetTop) {
			if (ComparePageObject.IE6)
				$("#left-nav").css({ position: "absolute", top: currScrollTop - 45, left: -ComparePageObject.MenuOffsetLeft + "px" });
			else {
				$("#left-nav").css({ position: "fixed", top: ComparePageObject.MenuOffsetTop + "px", left: (boxOffsetLeft - currScrollLeft - ComparePageObject.MenuOffsetLeft) });
			}
		} else {
			$("#left-nav").css({ position: "absolute", top: "232px", left: -ComparePageObject.MenuOffsetLeft + "px" });
		}
		$("#show-left-nav").hide();
	}
}
	...
	$.fn.clickChange = function (opts) {
		...
		this.each(function () {
			...
			_this.on('click', '.change-car-area .btn-pre-car', function () {
				...
				parentCloneElement.insertAfter(parent).css({ position: 'absolute', display: 'block', 'z-index': '1000' }).animate({ left: parentLeft - 164 }, option.speed, function () {
				...
				prevCloneElement.insertAfter(prevItem).css({ position: 'absolute', display: 'block', 'z-index': '999' }).animate({ left: parentLeft + 164 }, option.speed, function () {
			...
			_this.on('click', '.change-car-area .btn-next-car', function () {
				...
				nextCloneElement.insertAfter(nextItem).css({ position: 'absolute', display: 'block', 'z-index': '999' }).animate({ left: nextItemLeft - 164 }, option.speed, function () {
