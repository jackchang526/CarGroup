...
{
	public partial class _2012beijing_Serial : ExhibitionPageBase
	{
		...
		private string _SaveNewsUrl = "2012beijing";
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!ValidatorParam())
			{
				string url = "http://chezhan.bitauto.com/beijing/";
		...
		private void GetFocusContent()
		{
			...
			focusContent.AppendLine("<div class=\"car_indexbox\">");
			...
			//添加数字统计
			focusContent.AppendLine("<div class='tab'>");
		...
		private void GetVideoList()
		{
			...
			htmlContent.AppendFormat("<h3><span><a href='{0}' target='_blank'>{1} {2} 车展视频</a></span><small><em>{3}</em>条</small></h3>"
									, pc._Url, _BrandName, _SerialName, pc._Count);
			htmlContent.Append("<div class='car_listimg_zh car_listmovie_zh'><ul>");
			for (int i = 0; i < pc._NodeList.Count; i++)
			{
				...
				htmlContent.AppendFormat(" <a target='_blank' href='{0}'>{1}</a> <a target=\"_blank\" href=\"{0}\" class=\"ico_movie_small\"></a> ", newsUrl, title);
		...
		{
			string memcache = "Car_Exhibition_84_String_GetHotModelType";
		...
		private void InitTuJieXmlDocument()
		{
			string sPath = string.Format("http://imgsvr.bitauto.com/Photo/ImageService.aspx?"
										...
										+ "showaccount=true&serialid={0}&rownum=4&groupid=12&orderby=&imagename=chezhan-beijing2012", _SerialId);
			// 调整后的 图解地址 例如 
