...
$body.on('rightswipe3', function (ev, paras) {
    var setting = {
        ...
        flatFn1: function (data, paras) {
            data.forEach(function (curr, i) {
                curr.SaleStateCount = 0;
                curr.SellCount = 0;
                curr.Child.forEach(function (current, index) {
                    if (current.CsSaleState == '停销') {
                        curr.SaleStateCount++;

                    } else if (current.CsSaleState == '在销' || current.CsSaleState == '待销') {
                        curr.SellCount++;
                    }
                })
            })
    ...
    $body.trigger('publicswipe1', {
        ...
        dataCallBack: function (paras) {
            ...
            if ($brandlogo[0].className != 'undefined') {
            ...
            function swiperTabs() {
                ...
                sellCount = 0;
                var $this = this;
                var $content = $this.find('.swipe-content'),
                    $swiper = $content.find('.swiper-container');
                var as = $content.prev().find('a');
                as.each(function (index, current) {
                    (function ($o, i) {
                        $o.on('click', function (ev) {
                            ev.preventDefault();
                            mySwiper.slideTo(i);
                            $content.trigger('selectLabel', i);
                        })
                    })($(current), index);
                })
                $content.on('selectLabel', function (ev, i) {
                    as.parent().removeClass('current');
                    as.eq(i).parent().addClass('current');
                })
                ...
                var mySwiper = new Swiper($swiper[0], {
                    ...
                    onSlideChangeStart: function (swiper) {
                        $content.trigger('selectLabel', swiper.activeIndex);
                    }
                });
            ...
            /*绑定滚动插件*/
            var $content = $swipeLeft.find('.swipe-content');
            if ($content.length > 0) {
                swiperTabs.call($swipeLeft);
            } else {
                $swipeLeft.iScroll({ snap: options.snap });
            }
...
$body.on('publicswipe1', function (ev, paras) {
    ...
    $body.trigger('models', {
        ...
        closeEnd: function () {
            ...
            var $content = $swipeLeft.find('.swipeLeft-content');
            if ($content.length > 0) {
                $content.empty().append($loading.clone().children(0));
            } else {
...
$body.on('publicselect3', function (ev, paras) {
    var setting = {
        ...
        fliterTemplate2: function (templateid, f) {
            return f.$current.attr('data-tempate');
        },
        ...
        fliterTemplate3: function (templateid, f) {
            return f.$current.attr('data-tempate');
        },
        fliterData3: function (ds, f) {
            var data = [];
            for (var n in ds) {
                if (n.toString() == f.$current.attr('data-id').toString()) {
                    data = ds[n];
    ...
    $body.trigger(options.controlName1, {
        ...
        fnEnd: function (pms1) {
            ...
            $body.trigger(options.controlName2, {
                ...
                loading: options.loading,
                ds: options.ds2,
                fliterTemplate: options.fliterTemplate2,
                ...
                fnEnd: function (pms2) {
                    ...
                    $body.trigger(options.controlName3, {
                        ...
                        loading: options.loading,
                        ds: options.ds3,
                        fliterTemplate: options.fliterTemplate3,
                        fliterData: options.fliterData3,
