...
namespace BitAuto.CarChannel.CarchannelWeb.PageBrand
{
    public partial class CarBrandPage : PageBase
    {
        ...
        protected string brandTopNews = "";	//主品牌页顶部新闻
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            dictBrandBlockHtml = _commonhtmlBLL.GetCommonHtml(brandId, CommonHtmlEnum.TypeEnum.Brand, CommonHtmlEnum.TagIdEnum.BrandPage);
        ...
        private void RenderIntroduce()
        {
            ...
            BrandEntity brandEntity = (BrandEntity)DataManager.GetDataEntity(EntityType.Brand, brandId);
            if (brandEntity != null && brandEntity.Id > 0)
            {
                ...
                htmlCode.AppendLine(string.Format("<h3><span>{0}品牌故事</span></h3>", brandName));
                ...
                htmlCode.AppendLine(string.Format("<h3><span>{0}车标故事</span></h3>", brandName));
        ...
        private void RenderNewsNew()
        {
            ...
            try
            {
                ...
                //htmlTitle.AppendLine("<div class=\"h3_tab h3_tab_a\"><ul id=\"car_tab_BrandNews_ul\"><li class=\"current\"><a>最热</a></li>");

                int loop = 0;
                htmlCode.AppendFormat("<div id=\"data_table_BrandNews_{0}\">", loop.ToString());
                loop++;
                ...
                ////GetBrandNewsByCate(brandId, "新闻", ref loop, ref htmlTitle, ref htmlCode);
                //GetCateNewXmlContentNew(ds, "新闻", ref loop, ref htmlTitle, ref htmlCode);
                ////GetBrandNewsByCate(brandId, "行情", ref loop, ref htmlTitle, ref htmlCode);
                ////GetCateNewXmlContentNew(newsBll.GetTopBrandNews(brandId, CarNewsType.hangqing, 20),
                ////    "行情", ref loop, ref htmlTitle, ref htmlCode);
                ////降价
                //GetCateJiangJiaNews(ref loop, ref htmlTitle, ref htmlCode);
                ////GetBrandNewsByCate(brandId, "导购", ref loop, ref htmlTitle, ref htmlCode);
                //GetCateNewXmlContentNew(newsBll.GetTopBrandNews(brandId, CarNewsType.daogou, 20),
                //    "导购", ref loop, ref htmlTitle, ref htmlCode);
                ////GetBrandNewsByCate(brandId, "评测", ref loop, ref htmlTitle, ref htmlCode);
                //GetCateNewXmlContentNew(newsBll.GetTopBrandNews(brandId, CarNewsType.treepingce, 20),
                //    "评测", ref loop, ref htmlTitle, ref htmlCode);
                ////// GetBrandNewsByCate(brandId, "用车", ref loop, ref htmlTitle, ref htmlCode);
        ...
        private void RenderBrandSerials()
        {
            ...
            htmlCode.AppendLine("<div class=\"title-con\"><div class=\"title-box\"  id=\"seriallist\"><h2>" + brandName + "-全部车型</h2></div></div>");
            ...
            if (serialList.Count > 0)
            {
                foreach (CarSerialPhotoEntity serialEntry in serialList)
                {
                    ...
                    htmlCode.AppendLine("<a href=\"" + serilaUrl + "\" target=\"_blank\"><img src=\"" + imageUrl.Replace("_2.jpg", "_1.jpg") + "\" alt=\"" + serialEntry.CS_Name + "\" /></a>");
                    htmlCode.AppendLine("<div class=\"title\"><h3><a href=\"" + serilaUrl + "\" target=\"_blank\">" + serialEntry.CS_Name + "</a></h3></div>");
                    htmlCode.AppendLine("<div class=\"txt\">" + serialEntry.SaleState + "</div>");
                    htmlCode.AppendLine("</li>");

                    /*
                    string showName = serialEntry.ShowName.Replace("(进口)", "");
                    // modified by chengl Jun.3.2011
                    if (serialEntry.SerialId == 1568)
                    { showName = "索纳塔八"; }
                    ...
                    if (shortcsName != showName)
                        htmlCode.AppendLine("<p><b><a href=\"" + serilaUrl + "\" title=\"" + showName + "\" target=\"_blank\">" + shortcsName + "</a></b></p>");
                    else
                        htmlCode.AppendLine("<p><b><a href=\"" + serilaUrl + "\" target=\"_blank\">" + showName + "</a></b></p>");
                    // htmlCode.AppendLine("<p><a href=\"" + levelUrl + "\" target=\"_blank\">" + serialEntry.SerialLevel + "</a></p>");
                    
                    htmlCode.AppendLine("<p class=\"price\">" + serialEntry.SaleState + "</p>");
                    htmlCode.AppendLine("<p class=\"related\"><a target=\"_blank\" href=\"/" + serialEntry.CS_AllSpell + "/tupian/\" rel=\"nofollow\">图片</a>");
                    htmlCode.AppendLine("<a target=\"_blank\" href=\"/" + serialEntry.CS_AllSpell + "/koubei/\" rel=\"nofollow\">口碑</a>");
                    string bbsURL = (new Car_SerialBll()).GetForumUrlBySerialId(serialEntry.SerialId);
                    htmlCode.AppendLine("<a target=\"_blank\" href=\"" + bbsURL + "\" rel=\"nofollow\">论坛</a></p>");
            ...
            {
                // htmlCode.AppendLine("<div class=\"line_box zppbj\">");
                // htmlCode.AppendLine("<h3><span>" + brandName + "汽车停销车型</span></h3>");
                foreach (CarSerialPhotoEntity serialEntry in stopSaleList)
                {
                    ...
                    if (shortcsName.StartsWith(showName) || shortcsName.Length - showName.Length > 1)
                        shortcsName = showName;
                    if (shortcsName != showName)
                        htmlCode.AppendLine("<div class=\"title\"><h3><a href=\"" + serilaUrl + "\" target=\"_blank\">" + shortcsName + "</a></h3></div>");
                    else
                        htmlCode.AppendLine("<div class=\"title\"><h3><a href=\"" + serilaUrl + "\" target=\"_blank\">" + showName + "</a></h3></div>");


                    htmlCode.AppendLine("<div class=\"txt\"><small>停售</small>" + "</div>");
                    htmlCode.AppendLine("</li>");
            ...
            htmlCode.AppendLine("</ul></div>");//<div class=\"zppbj\"> end
