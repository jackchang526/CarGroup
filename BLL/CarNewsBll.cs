...
namespace BitAuto.CarChannel.BLL
{
    ...
    public class CarNewsBll
    {
        ...
        /// <param name="carNewsType">车型新闻类别</param>
        /// <returns></returns>
        public int GetSerialNewsCount(int serialId, CarNewsType carNewsType)
        {
            int result = 0;
            if (serialId <= 0)
            {
            ...
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetSerialNewsCount_{0}_{1}", serialId, (int)carNewsType);
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
                }
                else
                {
                    ...
                    result = newsDll.GetSerialNewsCount(serialId, arrCategroyIds);
        ...
        /// </summary>
        /// <param name="serialId">车系ID</param>
        /// <param name="carNewsTypeIdList">车型新闻分类ID List</param>
        /// <returns></returns>
        public int GetSerialNewsCount(int serialId, List<int> carNewsTypeIdList)
        {
            int result = 0;
            if (serialId <= 0 || !carNewsTypeIdList.Any())
            {
                return result;
            }
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetSerialNewsCount_{0}_{1}", serialId, string.Join("_", carNewsTypeIdList.ToArray()));
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
        ...
        public int GetMasterBrandNewsCount(int masterId, CarNewsType carNewsType)
        {
            int result = 0;
            if (masterId <= 0)
            {
            ...
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetMasterBrandNewsCount_{0}_{1}", masterId, (int)carNewsType);
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
                ...
                {
                    int[] arrSerialId = newsDll.GetSerialIdByMaster(masterId);
                    int[] arrCategroyIds = newsDll.GetCarNewsType((int)carNewsType);
                    result = newsDll.GetNewsCount(arrSerialId, arrCategroyIds);
                    CacheManager.InsertCache(cacheKey, result, WebConfig.CachedDuration);
                }
            }
            catch (Exception ex)
        ...
        /// <param name="carNewsTypeIdList"></param>
        /// <returns></returns>
        public int GetMasterBrandNewsCount(int masterId, List<int> carNewsTypeIdList)
        {
            int result = 0;
            if (masterId <= 0 || !carNewsTypeIdList.Any())
            {
                return result;
            }
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetMasterBrandNewsCount_{0}_{1}", masterId, string.Join("_", carNewsTypeIdList.ToArray()));
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
        ...
        /// <param name="carNewsType"></param>
        /// <returns></returns>
        public int GetBrandNewsCount(int brandId, CarNewsType carNewsType)
        {
            int result = 0;
            if (brandId <= 0)
            {
            ...
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetBrandNewsCount_{0}_{1}", brandId, (int)carNewsType);
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
                ...
                {
                    int[] arrSerialId = newsDll.GetSerialIdByBrand(brandId);
                    int[] arrCategroyIds = newsDll.GetCarNewsType((int)carNewsType);
                    result = newsDll.GetNewsCount(arrSerialId, arrCategroyIds);
                    CacheManager.InsertCache(cacheKey, result, WebConfig.CachedDuration);
                }
        ...
        /// <param name="carNewsTypeIdList"></param>
        /// <returns></returns>
        public int GetBrandNewsCount(int brandId, List<int> carNewsTypeIdList)
        {
            int result = 0;
            if (brandId <= 0 || !carNewsTypeIdList.Any())
            {
                return result;
            }
            try
            {
                string cacheKey = string.Format("Car_NewsBll_GetBrandNewsCount_{0}_{1}", brandId, string.Join("_", carNewsTypeIdList.ToArray()));
                object objCache = CacheManager.GetCachedData(cacheKey);
                if (objCache != null)
                {
                    result = (int)objCache;
