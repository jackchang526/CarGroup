...
namespace BitAuto.CarChannel.DAL
{
    public class Car_BrandDal
    {
        ...
        public List<CarSerialPhotoEntity> GetCarSerialPhotoListByCBID(int nCBID, bool isAll)
        {
            ...
            string sqlStr = @"SELECT cs.cs_id, cs_name, cs_ShowName,spell,allspell,CsSaleState,cs_CarLevel,cs_seoname 
                            FROM Car_Serial cs  WITH(NOLOCK)  
                            LEFT JOIN Car_Serial_30UV spv WITH(NOLOCK) ON  cs.cs_id=spv.CS_Id 
                            WHERE isstate = 1 AND cb_id = @cb_id ";
            if (!isAll) sqlStr += " AND CsSaleState<>'停销'";
            sqlStr += " ORDER BY cs.CsSaleState DESC,spv.PVCount DESC,cs.allspell";
