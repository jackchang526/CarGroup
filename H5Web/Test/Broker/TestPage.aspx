<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TestPage.aspx.cs" Inherits="H5Web.Test.Broker.TestPage" %>
...
<body>
    ...
    <div id="fullpage">
        ...
        <div class="section page11" data-anchor="page11">
            ...
            <!--大背景容器开始-->
            <div class="big_bg">
                <ins id="div_e86b3d36-3b41-4da9-8d7a-099e6960b100" type="ad_play" adplay_ip="" adplay_areaname="" adplay_cityname="" adplay_brandid="<%=serialId %>" adplay_brandname="" adplay_brandtype="" adplay_blockcode="e86b3d36-3b41-4da9-8d7a-099e6960b100"></ins>
                <% if (CommerceNewsList != null && CommerceNewsList.Count > 0)
                   { %>
                <ul class="car_article_list">
                    <% foreach (var news in CommerceNewsList)
                       {%>
                    <li>
                        <a href="<%=news.PageUrl.Replace("http://news.bitauto.com","http://news.h5.yiche.com") %>">
                            <% if (!string.IsNullOrEmpty(news.CarImage))
                               {
                            %>
                            <img src="<%=GetCompressImageUrl(news.CarImage,300) %>" />
                            <% }
                               else
                               { %>
                            <img src="http://img1.bitautoimg.com/default/150-100.gif" />
                            <% } %>
                            <span><%=news.FaceTitle %></span>
                            <p><%=news.PublishTime.ToString("yyyy-MM-dd") %><%=!string.IsNullOrEmpty(news.Author)?" / "+news.Author:string.Empty %></p>
                        </a>
                    </li>
                    <%}%>
