	...
	public partial class SuvChannel : PageBase
	{
		...
		private string RenderNewsListNew(int levelClassId, CarNewsType newsType)
		{
			...
			foreach (DataRow row in rows)
			{
				...
				htmlCode.AppendFormat("<li><a href=\"{0}\"target=\"_blank\" class=\"img\"><img src=\"{1}\"></a><p><a href=\"{0}\" target=\"_blank\">{2}</a></p>", newsUrl, pic, newsTitle);
				htmlCode.AppendFormat("<dl><dt>{0}{1}</dt><dd class=\"huifu comment_0_{3}\"></dd></dl></li>", author, publishTime.ToString("yyyy-MM-dd"), commentNum, newsId); ;
			}
		...
		private void RenderHotSuv()
		{
			...
			foreach (DataRow row in serialList.Take(8))
			{
				...
				if (priceRange.Trim().Length == 0)
					priceRange = "暂无报价";
				htmlCode.Append("<li>");
				htmlCode.AppendFormat("<a href=\"{0}\" target=\"_blank\"><img src=\"{1}\"/></a>", serialUrl, imgUrl.Replace("_2", "_1"));
				htmlCode.AppendFormat("<div class=\"title\"><a href=\"{0}\" target=\"_blank\">{1}</a></div>", serialUrl, serialName);
				htmlCode.AppendFormat("<div class=\"txt\">{0}</div></li>", priceRange);
				SerialNum++;
			}
			htmlCode.AppendLine("</ul>");
		...
		private void RenderSuvSalesRank()
		{
			...
			StringBuilder htmlCode = new StringBuilder();
			htmlCode.AppendLine("<ol class=\"rank-list\">");
