...
function mathTreeBoxHeight() {
	...
	var leftTreeHeight = winH + scrollHeight;
...
function getPavilionNavHtml(pavilionObj) {
	...
	for (var key in pavilionObj) {
		i++;
		html += "<li><a href=\"javascript:void(0);\" onclick=\"treeHref('ztreev1'," + i + ");return false;\">" + pavilionObj[key] + "</a></li>";
	}
	html += "</ul></div>";
	return html;
}
...
function getPavilionHtml(jsonTree) {
	...
	for (var pavilionId in jsonTree.pavilion) {
		...
		for (var j = 0; j < masterList.length; j++) {
			...
			html += "<li><a href=\"" + masterList[j].url + "\" class=\"" + className + "\"><img src=\"" + masterList[j].logo + "\" /><big>" + masterList[j].name + "</big></a></li>";
		}
		html += "</ul></li>";
	}
...
function getCharNavHtml(charObj) {
	...
	for (var key in charObj) {
		i++;
		if (charObj[key] == 1)
			html += "<li  class=\"t-" + key.toLowerCase() + "\"><a href=\"javascript:void(0);\" onclick=\"treeHref('treev1'," + i + ");return false;\">" + key + "</a></li>";
		else
...
function getBrandTreeHtml(jsonTree) {
	...
	html += "<div class=\"treev1\" id=\"treev1\" style=\"height: 400px;\"><ul>";
	var i = 0;
	for (var char in jsonTree.char) {
		i++;
		if (jsonTree.brand[char] == undefined)
			continue;
		html += "<li class=\"root\" id=\"treev1_" + i + "\"><b>" + char + "</b>";
		html += "<ul class=\"tree-item-box\">";
		//按字母输出主品牌
