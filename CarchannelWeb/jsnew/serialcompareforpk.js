...
$("#serial1,#serial2").change(function () {
    var id1 = $("#serial1").val();
    var id2 = $("#serial2").val();
    if (id1 != id2) {
        serialId1 = id1;
...
function SetSeeToSee() {
    if (seeToseeArr.length == 2) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            var url = arr[i].url;
            resultStr += "<li><a target=\"_blank\" href=\"" + url + "\">";
            resultStr += "<img src=\"" + imgurl + "\">";
            resultStr += "<div class=\"title\">" + serialName + "</div><span></span></a>";
            if (price != null && price.length > 0) {
                resultStr += "<div class=\"txt\"><em>" + price + "</em></div>";
            } else {
                resultStr += "<div class=\"txt huizi\"><em>暂无报价</em></div>";
            }
            resultStr += "</li>";
        }
        ...
        success: function (result) {
            ...
            if (result.length > 0) {
                for (var i = 0; i < result.length; i++) {
                    ...
                    resultStr += "<li>";
                    resultStr += "<a href=\"" + link + "\" class=\"img\" target=\"_blank\">";
                    resultStr += "<img src=\"" + firstimg + "\" width=\"210\" height=\"140\">";
                    resultStr += "</a>";
                    resultStr += "<div class=\"textcont\">";
                    resultStr += "<h3><a href=\"" + link + "\" target=\"_blank\">" + title + "</a></h3>";
                    resultStr += "<p class=\"info\"><span>来源：<em>" + source + "</em></span><span>作者：" +
                        "<em>" + author + "</em></span><span>" + publishtime + "</span>" +
                        "<span><em class=\"huifu\">" + commentcount + "</em></span></p>";
                    resultStr += "<p class=\"text\">";
                    resultStr += summary;
                    resultStr += "...<a href=\"" + link + "\" target=\"_blank\">查看更多&gt;&gt;</a>";
                    resultStr += "</p></div></li>";
                }
                if (resultStr.length > 0) {
...
function GetNewsPublishTime(dateTime) {
    ...
    var s = year + "-" + Appendzero(month) + "-" + Appendzero(day) + "&nbsp;" + "00:00";
    return s;
}
    ...
    if (carBaseInfoL != null && carBaseInfoR != null) {
        ...
        if (minPriceL > 0 && minPriceR > 0) {
            ...
            if (maxPriceL > 0 && maxPriceR > 0) {
                $("#ul_price").find("li[value=min]").removeClass();
                $("#ul_price").find("li[value=max]").addClass("current");
                SetPrice();
            }
        }
        //---
        ...
        if (minFuelL > 0 && minFuelR > 0) {
            ...
            if (maxFuelL > 0 && maxFuelR > 0) {
                $("#ul_fuel").find("li[value=min]").removeClass();
                $("#ul_fuel").find("li[value=max]").addClass("current");
                SetFuel();
            }
        }
    }
...
function SetPrice() {
    ...
    var carBaseInfoR = carBaseInfoArr[1];
    if (carBaseInfoL != null && carBaseInfoR != null) {
        var priceSelected = $("#ul_price").find("li[class=current]").attr("price");
        var priceL = 0;
        var priceR = 0;
        ...
        if (priceL > 0 && priceR > 0) {
            $("#price_l").html(priceL + "万");
            $("#price_r").html(priceR + "万");
...
function SetFuel() {
    ...
    var carBaseInfoR = carBaseInfoArr[1];
    if (carBaseInfoL != null && carBaseInfoR != null) {
        var fuelSelected = $("#ul_fuel").find("li[class=current]").attr("fuel");
        var fuelL = 0;
        var fuelR = 0;
    ...
    if (carBaseInfoL != null && carBaseInfoR != null) {
        ...
        if (maxSize > 0) {
            ...
            if (winCountL > 2 && winCountL>winCountR) {
                $("#div_space").find("a[name=carSerialName_l]").parent().parent().parent().addClass("win");
                $("#div_space").find("a[name=carSerialName_r]").parent().parent().parent().removeClass("win");
            }
            if (winCountR > 2 && winCountR>winCountL) {
                $("#div_space").find("a[name=carSerialName_r]").parent().parent().parent().addClass("win");
                $("#div_space").find("a[name=carSerialName_l]").parent().parent().parent().removeClass("win");
            }
            $("#div_space").show();
        }
    }
}
var maxLengthPx = 320; //px，车身尺寸，长度条的最大长度
function SetLengthPx(idStr, length, isWin) {
    var parentDiv = $("#" + idStr).parent();
    if (isWin) {
        $(parentDiv).addClass("win");
        $(parentDiv).find("div[class=baifenbi]").css({ background: "#c00", width: length + "px" });
    } else {
        $(parentDiv).removeClass("win");
        $(parentDiv).find("div[class=baifenbi]").css({ background: "#999", width: length + "px" });
    }
...
function SetImpress() {
    if (impressArr.length == 2) {
        ...
        if (impressL != null && impressR != null) {
            ...
            if (ratingL == ratingR) {
                $("#rating_l").parent().parent().parent().addClass("win");
                $("#rating_r").parent().parent().parent().addClass("win");
            }
            if (ratingL > ratingR) {
                $("#rating_l").parent().parent().parent().addClass("win");
                $("#rating_r").parent().parent().parent().removeClass("win");
            }
            if (ratingL < ratingR) {
                $("#rating_r").parent().parent().parent().addClass("win");
                $("#rating_l").parent().parent().parent().removeClass("win");
            }
            $("#div_koubei").show();
        }
    }
...
function SetImpressData(carType) {
    ...
    for (var i = 0; i < goodList.length; i++) {
        ...
        goodImpressStr += "<span>" + goodStr + "</span>";
    }
    for (var j = 0; j < badList.length; j++) {
        ...
        badImpressStr += "<span>" + badStr + "</span>";
    }
    goodImpressStr = goodImpressStr == "" ? "<span>暂无印象</span>" : goodImpressStr;
    badImpressStr = badImpressStr == "" ? "<span>暂无印象</span>" : badImpressStr;
    $("#" + youdianId).html(goodImpressStr);
    $("#" + quedianId).html(badImpressStr);
    $("#" + ratingId).html(rating + "分");
    var lenth = rating / maxAccount * 100;
    $("#" + ratingId).parent().find("div[class=mid_start_box]").find("em").css({ "width": lenth + "%" });
}
...
function showVote(data) {
    ...
    if (left >= right) {
        ...
        $("#vote-width .bft_left").addClass("bft_win");
        $("#vote-width .bft_right").removeClass("bft_win");

        $("#vote-width .bft_left").addClass("border_r").animate({ width: leftper + "%" }, 500, function () { }).html("<span>" + leftper + "%" + "</span>");
        $("#vote-width .bft_right").removeClass("border_l").css({ width: "" }).html("<span>" + rigthper + "%" + "</span>");
    } else {
        $("#con-sr").addClass("win");
        $("#con-sl").removeClass("win");
        $("#vote-width .bft_left").removeClass("bft_win");
        $("#vote-width .bft_right").addClass("bft_win");

        $("#vote-width .bft_left").removeClass("border_r").css({ width: "" }).html("<span>" + leftper + "%" + "</span>");
        $("#vote-width .bft_right").addClass("border_l").animate({ width: rigthper + "%" }, 500, function () { }).html("<span>" + rigthper + "%" + "</span>");
    }
}
//取Cookie
...
function fillImageBox(imageArray) {
    ...
    $.each(imageArray[0].imagelist, function (i, n) {
        ...
        slHtml.push("<li><a href=\"" + link + "\" " + target + "><img src=\"" + n.imgurl + "\" alt=\"\"></a></li>");
    });
    $("#photo-sl").html(slHtml.join(''));
    $.each(imageArray[1].imagelist, function (i, n) {
        var link = n.imglink == "" ? "javascript:;" : n.imglink;
        var target = n.imglink == "" ? "" : "target=\"_blank\"";
        srHtml.push("<li><a href=\"" + link + "\" " + target + "><img src=\"" + n.imgurl + "\" alt=\"\"></a></li>");
    });
