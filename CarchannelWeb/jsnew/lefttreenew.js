...
function treeBoxHeight() {
 ...
 var treeFixedNavHeight = parseInt(LeftTree.Tools.attrStyle(document.getElementById("treeFixedNav"), "height")) + 10; //10 =margin-bottom  导航栏高度
var bt_navigateNewHeight = parseInt(LeftTree.Tools.attrStyle(document.getElementById("header_logo"), "height")) + 10; //10 =margin-bottom 头部图标栏高度
//根据导航栏位置设置树形距顶高度
...
function MathScrollSettingTagBar() {
	...
	if (scrollHeight > topHeight) {
		carTagBar.className = "publicTabNew publicTabNew-fixed";
	} else {
		carTagBar.className = "publicTabNew";
	}
...
function treeHeight() {
	if (!IE6) {
		...
		setTimeout(function () { leftTreeBox.style.top = 39 + ie6AdHeight + "px"; }, 500);
	}
};
var ie6AdHeight = 0;
