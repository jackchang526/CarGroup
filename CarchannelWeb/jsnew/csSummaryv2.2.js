...
function scrollCarColor() {
    ...
    var normalMoveNum = parseInt(overColorNum / 3);
    ...
    var lastDistance = totalHeight - (allColorNum % 3) * itemHeight //最后一次移动的距离
...
var NewSummaryPageJavaScriptComplate = {
	favoritesUrlId: "addFavorites",
	favoritesTitle: "<%=serialSeoName %>-易车网",
	...
	{
		var control = document.getElementById(this.favoritesUrlId);
		if (!control) return;
		control.onclick = function () { NewSummaryPageJavaScriptComplate.FavoritesPage(); };
	},
	Init: function ()
	{
		with (this)
...
addLoadEvent(function () { NewSummaryPageJavaScriptComplate.Init(); });
