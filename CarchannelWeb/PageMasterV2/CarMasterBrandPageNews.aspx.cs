...
namespace BitAuto.CarChannel.CarchannelWeb.PageMasterV2
{
    ...
    public partial class CarMasterBrandPageNews : PageBase
    {
        ...
        private void InitNewsHeader()
        {
            ...
            foreach (var kv in titleTag)
            {
                ...
                if (kv.Key == CarNewsType.wenzhang)
                {
                    ...
                    newsCount = _carNewsBll.GetMasterBrandNewsCount(_MasterBrandId, carTypeIdList);
                }
                else
                {
                    newsCount = _carNewsBll.GetMasterBrandNewsCount(_MasterBrandId, kv.Key);
                }
                if (newsCount > 0)
                {
        ...
        private void InitNewsListNew()
        {
            ...
            {
                //newsType = "treepingce";
                ds = new CarNewsBll().GetMasterBrandNews(_MasterBrandId, CarNewsType.pingce, _PageSize, _PageIndex,
                    ref rowCount);
            }
            else
            {
