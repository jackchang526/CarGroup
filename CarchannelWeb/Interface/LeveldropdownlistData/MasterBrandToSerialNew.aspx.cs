	...
	*/
	public partial class MasterBrandToSerialNew : PageBase
	{
		...
		private string GetSerialContent()
		{
			...
			string cacheKey = string.Format("leveldropdownlistControl_{4}_{0}_{1}_{2}_{3}_{5}", _Conditions, _Include, _ParentId, _RequestType, _Serias, _DataName);
		...
		private string GetMasterBrandContent()
		{
			...
			{
				contentlist.Add(",");
				contentlist.Add(string.Format("\"m{0}\":", dr["bs_id"]));
				contentlist.Add("{");
		...
		private string GetCarTypeContent()
		{
			...
			CacheManager.InsertCache(cacheKey, jsonString.ToString(), 30);
