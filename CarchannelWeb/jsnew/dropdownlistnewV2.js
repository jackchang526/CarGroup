...
BindSelectNew.prototype.addDataItem = function (type, dataObj) {
    ...
    var thorldValue = 0, thorldList = {}, objCount = 0, maxLen = 0, groupObj = {}, groupStopObj = {}, tempText = "";
    var contentStop = [];
    for (var entity in dataObj) {
        ...
        if ((type == "master" || type == "producer") && thorldList[obj["tSpell"]] == null) {
            ...
            groupStopObj[preObj] = "";
            contentStop.push("<div class=\"pop-sub-tt\"><span>" + obj["goname"] + "</span></div>");
            objCount++;
        } else if (!(type == "master" || type == "producer" || type == "cartype") && groupObj[preObj] == null) {
            groupObj[preObj] = "";
            content.push("<div class=\"pop-sub-tt\"><span>" + obj["goname"] + "</span></div>");
            objCount++;
        }
        ...
        if (type == "cartype") {
            ...
            if (obj["salestate"] == "停销") {
                contentStop.push("<a href=\"javascript:;\" class=\"" + className + "\" bita-value=\"" + value + "\" bita-text=\"" + text + "\">");
                if (price != undefined || price == "") {
                    price = (price == "" || price == 0) ? "暂无" : price + "万";
                    contentStop.push("<strong>" + price + "</strong>");
                    tempText = price + text;
                }
                contentStop.push("" + text + "</a>");
            }
            else {
            ...
            if (type == "master" || type == "producer") {
                ...
                if (obj["csSale"] == "2") {
                    content.push("<a href=\"javascript:;\" title =\"" + text + "[停售]\" class=\"" + className + "\" bita-value=\"" + value + "\"bita-text=\"" + text + "\">" + text + "<span>停售</span></a>");
                }
                else {
                    content.push("<a href=\"javascript:;\" title =\"" + text + "\" class=\"" + className + "\" bita-value=\"" + value + "\"bita-text=\"" + text + "\">" + text + "</a>");
                }
            }
            else {
    ...
    if (contentStop.length > 0 && type == "cartype") {
        content.push("</div><div class=\"select-list f9\"><div class=\"pinp_main_unsale\">以下车款停售</div>");
