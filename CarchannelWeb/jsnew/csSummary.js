...
function ShowUCar(serialId, cityId) {
	var uCar = {
		parameters: "serialid=" + serialId + "&cityid=" + cityId,
		method: "get",
		onSuccess: function (res) {
			var ucar = document.getElementById("ucarInfo");
			if (res.responseText.length == 0)
				ucar.style.display = "none";
			else
				ucar.innerHTML = res.responseText;
		}
	}
	new Ajax.Request("/interface/iframeforucar.aspx", uCar);
}
...
function ShowVendorInfo(serialId, cityId) {
	var vendorOptions =
	{
		parameters: "csId=" + serialId + "&cityid=" + cityId,
		method: "get",
		onSuccess: onAjaxSuccess
	}
	new Ajax.Request("/interface/iframeForDealer.aspx", vendorOptions);
}
...
function onAjaxSuccess(res) {
	...
	if (res.responseText.length == 0)
		...
		vendor.innerHTML = res.responseText;
...
function next(previousBtn, nextBtn, scrollID) {
	...
	leftArr.onmouseover = function () { this.className = 'lHover' }
	leftArr.onmouseout = function () { this.className = 'l' }
	if (vLeft < -((allColor.length - 6) * 103)) {
		return moveing = false;
	}
	if (vLeft < -((allColor.length - 12) * 103)) {
	...
	var finalLeft = vLeft - 103 * 6;
	moveElement(scrollID, finalLeft, vTop, 10);
...
var ChangCityHangQing = {
	...
	cityId: '201',
	questUrl: 'http://car.bitauto.com/interfaceforbitauto/gethangqingnews.aspx?id=@serialId@&cityid=@cityid@',
