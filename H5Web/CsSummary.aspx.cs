...
namespace H5Web
{
	public partial class CsSummary : H5PageBase
	{
		...
		// 还关注Html带广告
		protected string AttentionSerialsHtml = string.Empty;
		...
		private void GetBrokerHtml(string serviceName)
		{
			...
			{
				string keyCache = "Car_H5_Broker_" + brokerid.ToString() + "_" + serialId.ToString();
		...
		{
			Dictionary<int, Dictionary<string, string>> serialAdList = base.GetSeeToSeeAD(serialId);
			Dictionary<int, string> dicAllCsPic = base.GetAllSerialPicURL(true);
			AttentionSerials = serialFourthStageBll.MakeSerialToSerialHtml(serialId);
			// 广告强制位置
			if (serialAdList != null && serialAdList.Count > 0)
			{
				for (int i = AttentionSerials.Count - 1; i >= 0; i--)
				{
					EnumCollection.SerialToSerial item = AttentionSerials[i];
			...
			if (AttentionSerials != null && AttentionSerials.Count > 0)
			{
				...
				foreach (EnumCollection.SerialToSerial sts in AttentionSerials)
				{
					if (loop > 4)
					{ break; }
					if (serialAdList != null && serialAdList.Count > 0
						&& serialAdList.ContainsKey(loop) && !listToID.Contains(ConvertHelper.GetInteger(serialAdList[loop]["AD_SerialID"])))
					{
						int otherID = ConvertHelper.GetInteger(serialAdList[loop]["AD_SerialID"]);
						listToID.Add(otherID);
						listTemp.Add(string.Format("<li><a href=\"{0}\">", serialAdList[loop]["Url"]));
						listTemp.Add(string.Format("<img src=\"{0}\">", (dicAllCsPic.ContainsKey(otherID) ? dicAllCsPic[otherID].Replace("_2.", "_3.") : WebConfig.DefaultCarPic)));
						listTemp.Add(string.Format("<span>{0}</span>", serialAdList[loop]["Title"]));
						if (!string.IsNullOrEmpty(serialAdList[loop]["PriceRange"]))
						{
							listTemp.Add(string.Format("<p>{0}</p>", serialAdList[loop]["PriceRange"]));
						}
						else
						{
							listTemp.Add(string.Format("<p>{0}</p>", GetSerialPriceRangeByID(otherID)));
						}
						listTemp.Add("</a></li>");
					}
					else
					{
						...
						{
							listToID.Add(sts.ToCsID);
							listTemp.Add(string.Format("<li><a href=\"/{0}/\">", sts.ToCsAllSpell));
							listTemp.Add(string.Format("<img src=\"{0}\">", sts.ToCsPic.Replace("_5", "_3")));
							listTemp.Add(string.Format("<span>{0}</span>", sts.ToCsShowName));
							listTemp.Add(string.Format("<p>{0}</p>", sts.ToCsPriceRange));
							listTemp.Add("</a></li>");
						}
					}
					loop++;
				}
				AttentionSerialsHtml = string.Join("", listTemp.ToArray());
			}
		}
