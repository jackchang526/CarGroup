(function () {
	...
	BitA.DropDownListNew({
		...
		onchange: { cartype: function () { addCarForSelect(); } }
	});
	//点击换车效果
	$("span[id^='btn-changecar-']").click(function (e) {
		if ($(this).hasClass("btn-show-car")) {
			...
			$(this).removeClass("btn-show-car").addClass("btn-hide-car").siblings(".popup-change-car").show();
			$("div.popup-change-car[bit-popupindex!='" + index + "']").hide();
			$("span[id^='btn-changecar-'][bit-changeindex!='" + index + "']").removeClass("btn-hide-car").addClass("btn-show-car");
		}
		else {
			$(this).removeClass("btn-hide-car").addClass("btn-show-car").siblings(".popup-change-car").hide();
		}
	});
	//请求 热门推荐车型
	...
	var headerTop = $("#img-compare-header").offset().top,
		boxLeft = $(".img-compare").offset().left,
		key = false;
	...
	$(window).scroll(function () {
		...
		if (scrollTop > headerTop) {
			...
			if (isIE6) {
				...
				$("#img-compare-header").css({ "position": "fixed", top: "0px", width: "990px", left: "50%", "margin-left": "-495px" });
				$(".left-nav").css({ "position": "fixed", top: "145px", left: boxLeft - 95 + "px" });
			}
		} else {
			key = false;
			$("#img-compare-header-placeholder").remove();
			$("#img-compare-header").css({ "position": "", top: "", width: "", left: "", "margin-left": "" });
			$(".left-nav").css({ "position": "", top: "", left: "" });
		}
	});
})();
//获取推荐车型
function getHotCarCompare(index, serialId) {
	$.ajax({ url: "/ajaxnew/gethotcarforphotocompare.ashx?serialIDs=" + serialId, cache: true, dataType: "json", success: function (data) {
		...
		for (var i = 0; i < data.length; i++) {
			if (checkCarIdExist(data[i].CarId)) continue;
			arrHtml.push("<li><a href=\"javascript:onchangeCarForSelect(" + index + "," + data[i].CarId + ");\"><strong>" + data[i].SerialName + "</strong> [换车]</a></li>");
			showCount++;
...
function changeBindSelect(index, mdvalue, sdvalue, cardvalue) {
	...
	BitA.DropDownListNew({
		...
		datatype: 5,
		callback: { cartype: function (data) { setCarDisabled(); } },
		onchange: { cartype: function () { var carId = $("#change-car-" + index + " .sel-item span").attr("value"); onchangeCarForSelect(index, carId); } }
	});
}
//检查 车款是否存在
...
function addCarForSelect() {
	var carId = $("#cartype4 .sel-item span").attr("value");
...
$(function () {
	...
	$(document).click(function (e) {
		...
		if ($(target).closest(".popup-change-car").length <= 0 && $(target).closest("span[id^='btn-changecar']").length <= 0) {
			$(".popup-change-car").hide();
			$("span[id^='btn-changecar-']").removeClass("btn-hide-car").addClass("btn-show-car");
		}
	});
	//设置不可选车型
	setCarDisabled();
...
function setCarDisabled() {
	if (carInfoJson && carInfoJson.length > 0) {
		for (var i = 0; i < carInfoJson.length; i++) {
			...
			$(".sel-item-box .popup-box a[bita-value='" + carId + "']").unbind("click").addClass("none");
		}
	}
}
//滚动监听
