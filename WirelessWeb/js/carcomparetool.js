	...
	for (var i = 0; i < ComparePageObject.ValidCount; i++) {
		...
		if (checkCarIsShowForeach(i)) {
			if (ComparePageObject.ArrCarInfo[i].CarInfoArray) {
				tempArray.push("<td width=\"30%\" id=\"td_" + i + "\" ><div class=\"car-item car-item-gray\"><span>");
				try {
					// car info
					...
					tempArray.push(csShowName + " " + (carYear == "" ? "" : " " + carYear + "款 ") + carName + "</span>");
...
$(window).scroll(function () {
	...
	if (scrollTop > 40 && !$("#container").is(":hidden")) {
		$("#popMenu").show();
	} else {
		$("#popMenu").hide();
	}
