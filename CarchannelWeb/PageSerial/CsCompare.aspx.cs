...
namespace BitAuto.CarChannel.CarchannelWeb.PageSerial
{
	public partial class CsCompare : PageBase
	{
		...
		// private StringBuilder sbAllCarJson = new StringBuilder();
		// protected string PageAllCarJson = string.Empty;
		...
		private void GetCarIDByCsID()
		{
			if (csID > 0)
			{
				...
				// modified by chengl May.15.2012
				if (cbe.SaleState == "停销")
				{
					// 停销子品牌取最新年款的车型
				...
				if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
				{
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
					{
						...
						// 页面json
						// GetAllCarParamToJson(int.Parse(ds.Tables[0].Rows[i]["car_id"].ToString()), ds.Tables[0].Rows[i]["car_name"].ToString().Trim(), ref sbAllCarJson);
					}
					// PageAllCarJson = sbAllCarJson.ToString();
					// sbAllCarJson.Remove(0, sbAllCarJson.Length);
				}
				//bool isSuccess = false;
				//CsHeadHTML = this.GetRequestString(string.Format(WebConfig.HeadForSerial, csID.ToString(), "CsCompare"), 10, out isSuccess);
				if (year > 0)
				{
