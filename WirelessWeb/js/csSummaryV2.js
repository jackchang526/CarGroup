	...
	var mCsSummaryV2 = (function () {
				...
				success: function (data) {
					...
					$.each(data, function (i, n) {
						if (!(n.StateSubsidies > 0 && n.LocalSubsidy > 0)) return;
 						var b = "<em class=\"bt\">补贴</em>";
 						var carLine = $("#carlist_" + n.CarId + " .tap-box").prepend(b);						 
					});
				}
			});
		}
		...
		s.init = function () {
			...
			$("#btn-hot-more,#btn-pingce-more,#btn-daogou-more").one("click", function () {
				...
				swiperNewslist.updateAutoHeight();
				$(this).hide();
				...
				if (curBtnStr.indexOf("hot") > -1)
					...
					curMoreLink = 'daogou';
				$(this).after("<a href=\"http://car.m.yiche.com/" + _csAllSpell + "/" + curMoreLink + "/\"  class=\"btn-more\"><i>查看更多</i></a>");
			});
		};
		s.init();
		return s;
