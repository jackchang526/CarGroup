...
{
	public class PingCeTree : NewsTreeData
    {
        private string _TagType = "treepingce";
		public PingCeTree()
		{
			base.carNewsType = Common.Enum.CarNewsType.treepingce;
		}
        ...
        {
            string filePath = HttpContext.Current.Server.MapPath("/include/09gq/09gq_kan/09gq_pingce/00001/09gq_kc_pcsy_jdt_Manual.shtml");
        ...
        {
            string filePath = HttpContext.Current.Server.MapPath("/include/09gq/09gq_kan/09gq_pingce/00001/09gq_kc_pcsy_jdq_Auto.shtml");
            if (!File.Exists(filePath))
            {
                return "";
            }
