...
namespace H5Web.cssummary.v6
{
    public partial class CsSummary : H5PageBase
    {
        ...
        private void InitCustomizationType()
        {
            ...
            if (Brokerid > 0)
            {
                CurrentCustomizationType = V6.CustomizationType.Broker;
            }

            Dealerid = ConvertHelper.GetInteger(Request.QueryString["dealerid"]);
            if (Dealerid > 0)
            {
                CurrentCustomizationType = V6.CustomizationType.Dealer;
            }
            Chezhuka = Request.QueryString["chezhukamark"];
            if (Chezhuka == "chezhuka")
            {
                CurrentCustomizationType = V6.CustomizationType.CheZhuKa;
            }
            ...
            if (DealerPersonId > 0)
            {
                CurrentCustomizationType = V6.CustomizationType.DealerSale;
            }

            AgentId = ConvertHelper.GetInteger(Request.QueryString["agentid"]);
            if (AgentId > 0)
            {
                CurrentCustomizationType = V6.CustomizationType.HuiMaiCheGuWen;
            }
        }
        ...
        protected V6.CustomizationType CurrentCustomizationType = V6.CustomizationType.User;
