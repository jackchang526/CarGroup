...
function treeBoxHeight() {
	...
	var header = document.getElementById("tree_header");
	var headerTop = LeftTree.Tools.getOffsetTop(header); //获取导航栏距离顶部距离
	var headerHeight = LeftTree.Tools.attrStyle(header, "height");

	//if (scrollTop >= (headerTop + headerHeight)) { scrollHeight += treeFixedNavHeight; } //fixed 影响滚动高度
	var leftTreeHeight = winH - headerTop + scrollTop;
...
function treeFixedNavTop() {
	...
	var scrollTop = document.body.scrollTop || document.documentElement.scrollTop;
 	if (scrollTop >= (headerTop + parseInt(headerHeight))) {
 		document.getElementById("leftTreeBox").style.top = 0 + "px";
		return;
	}
	document.getElementById("leftTreeBox").style.top = headerTop + headerHeight - scrollTop + "px";
}
//计算滚动对导航栏和设置
...
function treeHeight() {
	if (!IE6) {
		...
		var leftTreeBox = document.getElementById("leftTreeBox");
		var leftTreeHeight = LeftTree.Tools.getOffsetTop(leftTreeBox);
		setTimeout(function () { leftTreeBox.style.top = leftTreeHeight + ie6AdHeight + "px"; }, 500);
	}
};
var ie6AdHeight = 0;
...
function JsonpCallBack(data) {
	...
	LeftTree.Tools.addEvent(window, "scroll", function () {
		if (!IE6) {
			...
			leftTreeBox.style.top = leftTreeBox + ie6AdHeight + "px"; //29:登陆条高度
		}
	}, false);
	treeHeight();
}
...
function pullTopAd(adHeight) {
	...
	if (IE6) {
 	...
 	document.getElementById("leftTreeBox").style.top = leftTreeBox + adHeight - 10 + "px";
}
