...
var mSerialSummary = (function (module) {
	var self = module;
	//加载行情
	module.loadNewsHangqingV2 = function (serialId, serialName, cityId, count) {
		$.ajax({
			url: "http://m.h5.qiche4s.cn/jiangjiaapi/GetPromtionNews.ashx?csid=" + serialId + "&cid=" + cityId + "&op=promotionnews", cache: true, dataType: "jsonp", jsonpCallback: "getHQCallback", success: function (data) {
				var h = [];
				if (data && data.length > 0) {
					h.push("<ul>");
					$.each(data, function (i, n) {
						if (i >= 4) return;
						h.push("<li><a href=\"" + n.URL + "\"><p>" + n.newsTitle + "</p><em>" + n.NewsPubTime + "</em></a></li>");
					});
					h.push("</ul>");
				} else {
...
(function () {
	...
	//统计
	var channelIDs = { "9": "27.23.119", "10": "27.23.120", "11": "27.23.121", "12": "27.23.122", "13": "27.23.123", "14": "27.23.994" };
	var urlEndPartCode = { "9": "&tracker_u=18_ycydcx&leads_source=m002004", "10": "&source=100064&leads_source=m002005", "11": "&ref=mcar1&rfpa_tracker=2_22&leads_source=m002006", "12": "?ref=mchexizska&leads_source=m002007", "14": "?leads_source=m002017" };
	//购车服务
	$.ajax({
		url: "http://api.car.bitauto.com/api/GetBusinessService.ashx?action=mserial&cityid=" + cityId + "&serialid=" + CarCommonCSID,
		async: false,
		dataType: "jsonp",
				...
				if (data.BigButton) {
					...
					for (i = 0; i < forend; i++) {
						...
						if (data.BigButton[i].BusinessId == "13") { //购车服务
							...
							serviceHtml.push("<a  data-channelid=\"" + curChannelId + "\" href=\"http://gouche.m.yiche.com/\"><strong class=\"icon-66\">&nbsp;</strong>");
							serviceHtml.push("<em>购低价 购放心</em>");
						}
						else {
