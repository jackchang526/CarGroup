...
(function ($) {
    ...
    BindSelectNew.prototype.setDefaultValue = function (type) {
        if (type == "master") {
            ...
            var content = "<span class=\"default\">" + this.getDefaultValue(type).text + "</span><a href=\"javascript:;\" class=\"jt\"></a><div class=\"zcfcbox clearfix\" style=\"display:none;\"><div class=\"brand_bg models_bg style_bg brand_price\" id=\"" + type + "div\" style=\"display: block;\"><div class=\"models_detail_bg\" style=\"height:330px;\"><div class=\"models_detail\"></div></div></div></div>";
            $("#" + this.defaults.container[type]).unbind("click").html(content);
        }
    }
    //获取数据
    ...
    BindSelectNew.prototype.addDataItem = function (type, dataObj) {
        ...
        for (var entity in dataObj) {
            ...
            if (type == "cartype") {
                ...
                price = (price == "undefined" || price == "" || price == 0) ? "暂无" : price + "万";

                if (obj["salestate"] == "停销") {
                    contentStop.push("<dd><a href=\"javascript:;\" title=\"" + text + "\"  bita-value=\"" + value + "\" bita-text=\"" + text + "\" bita-price=\"" + price + "\">");
                    //if (price != undefined || price == "") {
                    //    price = (price == "" || price == 0) ? "暂无" : price + "万";
                    //    contentStop.push("<strong>" + price + "</strong>");
                    //    tempText = price + text;
                    //}
                    contentStop.push("" + text + "<strong>" + price + "</strong></a></dd>");
                } else {
                    content.push("<dd><a href=\"javascript:;\" title=\"" + text + "\"  bita-value=\"" + value + "\" bita-text=\"" + text + "\" bita-price=\"" + price + "\">");
                    //if (price != undefined || price == "") {
                    //    price = (price == "" || price == 0) ? "暂无" : price + "万";
                    //    content.push("<strong>" + price + "</strong>");
                    //    tempText = price + text;
                    //}
                    content.push("" + text + "<strong>" + price + "</strong></a></dd>");
                }
            } else {
                //如果该类型为第一级
                if (type == "master" || type == "producer") {
                    ...
                    content.push("<dd><a href=\"javascript:;\" bita-value=\"" + value + "\" bita-text=\"" + text + "\" title=\"" + text + "\">" + obj["tSpell"] + " " + text + "</a></dd>");
                    if (!charList.contains(obj["tSpell"])) {
                        charList.push(obj["tSpell"]);
                    }
                } else if (type == "serial") {
                    if (obj["csSale"] == "2") {
                        content.push("<dd><a href=\"javascript:;\" bita-value=\"" + value + "\" bita-text=\"" + text + "\" title=\"" + text + "\">" + text + "<span>停售</span></a></dd>");
                    } else {
                        content.push("<dd><a href=\"javascript:;\"  bita-value=\"" + value + "\" bita-text=\"" + text + "\" title=\"" + text + "\">" + text + "</a></dd>");
                    }
                } else
                    content.push("<dd><a href=\"javascript:;\" bita-value=\"" + value + "\" bita-text=\"" + text + "\" title=\"" + text + "\">" + text + "</a></dd>");
            }
