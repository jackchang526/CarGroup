...
{
	public class ExhibitionPageBase : PageBase
	{
		...
		public struct ExhibitionBaseInfo
		{
			...
			public string CMSNews;							// 新闻分类接口地址
		}
							...
							foreach (XmlNode xn in xnl)
							{
								...
								if (int.TryParse(xn.Attributes["id"].Value.ToString(), out eid))
								{
									if (eid > 0 && !_DicExhibitionBaseInfo.ContainsKey(eid))
									{
										...
										XmlNode cmsModelUrlNode = xn.SelectSingleNode("./cmsModelUrl");
										if (cmsModelUrlNode != null && cmsModelUrlNode.Attributes["url"] != null)
										{ ebi.CMSModelUrl = cmsModelUrlNode.Attributes["url"].Value; }

										XmlNode newcarUrlNode = xn.SelectSingleNode("./newcarUrl");
										if (newcarUrlNode != null && newcarUrlNode.Attributes["url"] != null)
										{ ebi.NewCarUrl = newcarUrlNode.Attributes["url"].Value; }
										...
										XmlNodeList xnlCMSNewsUrl = xn.SelectNodes("CMSNews");
										if (xnlCMSNewsUrl != null && xnlCMSNewsUrl.Count > 0 && xnlCMSNewsUrl[0].Attributes["url"] != null)
										{ ebi.CMSNews = xnlCMSNewsUrl[0].Attributes["url"].Value; }
		...
		private void IntiSerialJieXiURL()
		{
			...
			string cacheKey = "ExhibitionPageBase_IntiSerialJieXiURL_" + _ExhibitionID;
			...
			if (intiSerialJieXiURL == null)
			{
				...
				if (_DicExhibitionBaseInfo.ContainsKey(_ExhibitionID)
					&& !string.IsNullOrEmpty(_DicExhibitionBaseInfo[_ExhibitionID].CMSNews))
				{
					doc.Load(_DicExhibitionBaseInfo[_ExhibitionID].CMSNews);
				}
		...
		private void BuilderAttributeUrlList()
		{
			...
			if (getAllAttributeByExhibitionIdUrlReverse == null)
			{
				...
				if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
				{
					for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
					{
						if (!AttributeUrlReverse.ContainsKey(Convert.ToString(i + 1)))
						{ AttributeUrlReverse.Add(Convert.ToString(i + 1), ds.Tables[0].Rows[i]["ID"].ToString().Trim()); }
					}
				}
		...
		protected List<string> GetLevelStringList(string allSpell)
		{
			...
			if (allSpell != "")
			{
				...
				foreach (string level in arrTemp)
				{
					...
					switch (level)
					{
						case "paoche": name = "跑车"; break;
						case "weixingche": name = "微型车"; break;
						case "xiaoxingche": name = "小型车"; break;
						case "jincouxingche": name = "紧凑型"; break;
						case "zhongxingche": name = "中型车"; break;
						case "zhongdaxingche": name = "中大型"; break;
						case "mianbaoche": name = "面包车"; break;
						case "haohuache": name = "豪华车"; break;
					}
