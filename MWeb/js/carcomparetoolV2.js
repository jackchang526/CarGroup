						...
						if (colorArray.length > 0) {
							...
							for (var icolor = 0; icolor < colorArray.length; icolor++) {
								...
								if (colorRGB.length == 2) {
									tempColor.push("<li><a href=\"###\" class=\"c-" + colorRGB[1] + "\"></a></li>");
								}
							}
							tempColor.push("</ul>");
...
function selectCarId(carId, carName, currentIndex) {
	...
	//ComparePageObject.arrCarIds.push(carId);
	$.ajax({
		url: "http://api24.car.bitauto.com/CarInfo/GetCarParameter.ashx?isParamPage=1&carids=" + ComparePageObject.arrCarIds.join(","), dataType: "script", cache: true, success: function (data) {
