...
{
    public partial class CsSummary : WirelessPageBase
    {
        ...
        private void MakeForumNewsHtml()
        {
            ...
            sb.Append("<h3>论坛热帖</h3>");
            sb.AppendFormat("<div class='opt-more opt-more-gray'><a href='{0}'>更多</a></div>", _baaUrl);
            sb.Append("</div>");

            sb.Append(" <div class='card-news' id='m_hotforum' data-channelid=\"27.23.737\">");
            sb.Append("<ul>");
            int i = 0;
            foreach (XmlNode node in newsList)
            {
                ...
                sb.AppendFormat("<h4>{0}</h4>", newsTitle);
                ...
                if (imglistNode != null)
                {
                    ...
                    if (xmlNodeList != null && xmlNodeList.Count > 0)
                    {
                        ...
                        foreach (XmlNode item in xmlNodeList)
                        {
                            ...
                            sb.AppendFormat("<li><span><img src='{0}' alt=\"{1}\" width='216px' heigth='144px'/></span></li>",
        ...
        private void InitSerialInfo()
        {
            ...
            if (_serialInfoCard.CsSaleState == "停销")
                _serialPrice = "停售";
            else if (_serialInfoCard.CsPriceRange == null || _serialInfoCard.CsPriceRange.Length == 0)
                _serialPrice = "暂无报价";
            else
        ...
        private void SerialNews()
        {
            ...
            if (dictSerialBlockHtml.ContainsKey(focusNewsForWireless))
                _serialNews = dictSerialBlockHtml[focusNewsForWireless];
        }
        ...
        private void MakeSerialToSerialHtml()
        {
            ...
            htmlTagCode.Append("<div class='tt-first' id='m-tabs-kankan'><ul class='tags-sub tags-sub-left tags-sub-left-tt'>");
            ...
            {
                firstTag = true;
                htmlTagCode.Append(" <li class='current'><a href='#'>看了还看</a></li>");
                ...
                if (flag)
                {
                    ...
                    foreach (var info in listGroupNew)
                    {
                        ...
                        foreach (CarInfoForSerialSummaryEntity carInfo in carGroupList)
                        {
                            ...
                            stringBuilder.Append("<ul class='car-btn'>");
                            stringBuilder.AppendFormat(
                                "<li><a id=\"car-compare-{0}\" href=\"#compare\" data-action=\"car\" data-id=\"{0}\" data-name=\"{1} {2}\" data-channelid=\"27.23.910\">加入对比</a></li>",
                                carInfo.CarID, _serialName, carInfo.CarName);
                            stringBuilder.AppendFormat(
                    ...
                    if (pageCount > 1)
                    {
                        stringBuilder.AppendFormat("<a id='btnLoadNext{0}' page='2'  class='btn-more btn-add-more' href='javascript:void(0);'><i>加载更多</i></a>", counter);
                    }
                }
                flag = false;
                stringBuilder.Append("</div>");
