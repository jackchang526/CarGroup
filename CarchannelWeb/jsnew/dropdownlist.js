...
BindSelect.prototype.BindList = function (type, parentDataId) {
	...
	{
		//指定父ID
		parentDataId = this.defaults.parent[type];
		if (parentDataId == undefined)
			parentDataId = 0;
		this.GetDataList(type, parentDataId); //通地createscript方式得到数据
		return;
	}
	...
	this.CallbackData(type, isOptionObject);
}
//回调函数
BindSelect.prototype.CallbackData = function (type, data) {
...
BindSelect.prototype.AddOption = function (type, dataObj) {
	...
	for (var entity in dataObj) {
		...
		//如果父类超过1个，并且没有创建组 主品牌只有一个品牌不显示
		if ((thorld > 1 || (type != "serial" && thorld >= 1)) && tempParentList[preObj] == null) {
...
BindSelect.prototype.GetDataList = function (type, parentDataId) {
	...
	var pro = this;
	loadJS.push(url, this.defaults.encode, function () { pro.CallBackOpertion(type, parentDataId, objName); });
}
//处理异步请求成功后，程序要进行的过程
BindSelect.prototype.CallBackOpertion = function (type, parentDataId, objName) {
	...
	//回调方法
 	this.CallbackData(type, bindData);
}
//得到绑定值
...
//设置选中
BindSelect.prototype.setItemDefaultValue = function (type) {
...
BitA.DropDownList = function (options) {
	if (!options) options = {};
	var bindSelect = new BindSelect(options);
	bindSelect.BindList();
	return bindSelect;
};