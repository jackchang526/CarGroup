...
namespace BitAuto.CarChannelAPI.Web.Level
{
	/// <summary>
	/// GetCarLevelHtml 的摘要说明
	/// </summary>
	public class GetCarLevelHtml : IHttpHandler
	{
		private int level = 1;
		...
		public void ProcessRequest(HttpContext context)
		{
			...
			GetParams(context);
			context.Response.Write("var levelHtml='" + GetInnerHTML() + "'");
		}
		...
		private void GetParams(HttpContext context)
		{
			...
			levelName = CarUtils.Define.CarLevelDefine.GetLevelNameById(level);
		}
		/// <summary>
		/// 得到HTML内容
		/// </summary>
		...
		private string GetInnerHTML()
		{
			...
			{
				if (serialNode.GetAttribute("CsLevel") == levelName)
		...
		private string RenderSerialByBrand()
		{
			...
			foreach (XmlElement mbNode in mbNodeList)
			{
				...
				//判断是否有当前级别的子品牌
				XmlNodeList serialList = mbNode.SelectNodes("Brand/Serial");
				foreach (XmlElement element in serialList)
				{
					if (element.GetAttribute("CsLevel") == levelName)
					{
						serialFlag = true;
						break;
					}
			...
			foreach (string fChar in CommonFunction.CharList)
			{
				...
				foreach (XmlElement mbNode in mbDic[fChar])
				{
					mbCounter++;
					//生成主品牌图标与链接
					string mbUrl = "/" + mbNode.GetAttribute("AllSpell").ToLower() + "/";
		...
		private void RenderBrands(List<string> htmlCode, XmlElement mbNode)
		{
			...
			{
				//判断是否有当前级别的子品牌
				XmlNodeList serialList = ele.SelectNodes("Serial");
				foreach (XmlElement element in serialList)
				{
					if (element.GetAttribute("CsLevel") == levelName)
					{
