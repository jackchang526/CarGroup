...
namespace BitAuto.CarChannel.CarchannelWeb.CarTree
{
	public partial class CX_masterbrand : TreePageBase
	{
		...
		private void GetNavbarHtml()
		{
			...
			sbNavbar.AppendFormat("    	<div><span>您当前的位置：</span><a href=\"http://www.bitauto.com/\">易车</a> &gt; <a href=\"http://car.bitauto.com/\">车型</a> &gt; <strong>{0}</strong></div>", _MasterBrandName);
			sbNavbar.Append("</div>");
		...
		private void GetMasterContainsBrandList()
		{
				...
				foreach (KeyValuePair<string, List<DataRow>> kvp in dicCs)
				{
					...
					foreach (var group in query)
					{
						...
						if (_MasterBrandEntity.BrandList.Length > 1)
							sbCsList.AppendFormat("<div class=\"title-con-2\"><h5><a href=\"/tree_chexing/b_{0}/\">{1}&gt;&gt;</a></h5></div>",
									key.BrandId, brandName);
						sbCsList.AppendLine("<div class=\"carpic_list\"><ul>");
						...
						foreach (DataRow dr in groupList)
						{
                            ...
                            if (sellState.Trim() == "停销")
                            {
                                sbTempList.AppendLine(string.Format(
                                   "<li><a href=\"/{0}/\" title=\"{3}\" target=\"_blank\" stattype=\"car\" id=\"n{1}\"><img src=\"{2}\" alt=\"{3}\"></a>"
                                   , csSpell, serialId, imgUrl, csShowName));
                                sbTempList.AppendLine(string.Format("<div class=\"title\"><a href=\"/{0}/\" title=\"{1}\" target=\"_blank\" stattype=\"car\" id=\"m{2}\">{1}</a>{4}{5}</div><div class=\"txt{6}\">{3}</div>"
                                    , csSpell, csShowName, serialId, priceRang, subsidiesString, bestCarStr,
                                   priceRang == "暂无报价" ? " huizi" : ""));
                                sbTempList.AppendLine("</li>");
                            }
                            else
                            {
                                sbCsList.AppendLine(string.Format(
                                    "<li><a href=\"/{0}/\" title=\"{3}\" target=\"_blank\" stattype=\"car\" id=\"n{1}\"><img src=\"{2}\" alt=\"{3}\"></a>"
                                    , csSpell, serialId, imgUrl, csShowName));
                                sbCsList.AppendLine(string.Format("<div class=\"title\"><a href=\"/{0}/\" title=\"{1}\" target=\"_blank\" stattype=\"car\" id=\"m{2}\">{1}</a>{4}{5}</div><div class=\"txt{6}\">{3}</div>"
                                    , csSpell, csShowName, serialId, priceRang, subsidiesString, bestCarStr,
                                   priceRang == "暂无报价" ? " huizi" : ""));
			...
			listHTML.Add(string.Format("<h3><a href=\"/{0}/\" target=\"_blank\" class=\"title-logo\"><img src=\"http://image.bitautoimg.com/bt/car/default/images/logo/masterbrand/png/30/m_{2}_30.png\"></a><a href=\"/{0}/\" target=\"_blank\">{1}</a></h3>", _MasterBrandEntity.AllSpell
				, _MasterBrandEntity.Name.Length >= 4 ? _MasterBrandEntity.Name : _MasterBrandEntity.Name, _MasterBrandId));
			listHTML.Add(sbTitle.ToString());
            listHTML.Add(string.Format("<div class=\"more\"><a href=\"/{0}/\" target=\"_blank\">{1}频道</a> | <a href=\"http://www.taoche.com/{0}/?leads_source=p012001\" target=\"_blank\">二手车</a></div>", _MasterBrandEntity.AllSpell, _MasterBrandEntity.Name));
			listHTML.Add("</div>");
			listHTML.Add("</div>");
