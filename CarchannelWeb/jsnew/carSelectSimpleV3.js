...
var CarSelectSim = {
    ...
    CarDataParms: "type={type}&pid={pid}&rt={rt}&v=20160120",
    DataType: "master",
    Callback: null,
    MasterName: "",
    MasterId: "",
    ...
    GetSerial: function (pid, pname, e) {
        ...
        var url = self.CarDataUrl + self.format(self.CarDataParms, { "type": "1", "pid": pid, "rt": "serial" });
                ...
                if (child && child.length > 0) {
                    for (var j = 0; j < child.length; j++) {
                        if (child[j]["saleState"] == "停销") {
                            tempHtml.push("<p><a href=\"javascript:;\" sid=\"" + child[j]["id"] + "\" sname=\"" + child[j]["name"] + "\" name=\"serial\">" + child[j]["name"] + "<span>停售</span></a></p>");
                        }
                        else {
    ...
    GetCarType: function (pid, pname, e) {
        ...
        var url = self.CarDataUrl + self.format(self.CarDataParms, { "type": "0", "pid": pid, "rt": "cartype", "callback": "CarSelectSim.FillCarType" });
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i]["nosale"] == "true") {
                if (hasNoSaleFlag == false) {
                    tempHtml.push("<div class=\"pinp_main_unsale\">以下车款停售</div>");
                    hasNoSaleFlag = true;
                }
                tempHtml.push("<div class=\"pinp_main_zm f9\">");
            }
            else {
                tempHtml.push("<div class=\"pinp_main_zm\">");
            }
